        ...
        for i in range(4):
            for j in range(4):
                for k in range(4):
                    ...
                    ll = torch.tensor([x_min + 0.25 * i * x_d, y_min + 0.25 * (j) * y_d, z_min + 0.25 * (k) * z_d]).to(
                        PC1.device)
                    ur = torch.tensor(
                        [x_min + 0.25 * (i + 1) * x_d, y_min + 0.25 * (j + 1) * y_d, z_min + 0.25 * (k + 1) * z_d]).to(
                        PC1.device)
                    # print(ll,ur)
