...
def number_of_features_per_level(init_channel_number, n_levels):
    return [init_channel_number * 2 ** k for k in range(n_levels)]


class SingleConv(nn.Sequential):
    """
        Basic convolutional module consisting of a 3D convolution, non-linearity and optional batchnorm/groupnorm.
        ...
        for i, char in enumerate(order):
            if char == 'r':
                ...
                assert num_channels % num_groups == 0, (
                    f'Expected number of input channels to be divisible by num_groups.'
...
class DoubleConv(nn.Sequential):
    """
        ...
        Args:
            ...
            kernel_size (int): size of the convolutional kernel
            order (string): determines the order of layers in `SingleConv` module. See `SingleConv` for more info.
            num_groups (int): number of groups for the GroupNorm
    """
...
class Upsampling(nn.Module):
    """
        Upsamples a given multichannel 3D data using either interpolation or learned transposed convolution.
...
class Encoder(nn.Module):
    """
        ...
        from the standard (2,2,2), e.g. if the volumetric data is anisotropic, also
...
class Decoder(nn.Module):
    """
        ...
        (either learned ConvTranspose3d or nearest neighbor interpolation) followed by a basic module
        (DoubleConv or ExtResNetBlock).
        Args:
            in_channels (int): number of input channels
            out_channels (int): number of output channels
            kernel_size (int): size of the convolutional kernel
...
class Abstract3DUNet(nn.Module):
    """
        ...
        Args:
            ...
            num_groups (int): number of groups for the GroupNorm
            n_levels (int): number of levels in the encoder/decoder path (applied only if f_maps is an int)
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
                 num_groups=8, n_levels=4, is_segmentation=False, testing=False, **kwargs):
        super(Abstract3DUNet, self).__init__()

        self.testing = testing

        if isinstance(f_maps, int):
            f_maps = number_of_features_per_level(f_maps, n_levels=n_levels)
    ...
    def forward(self, x):
        ...
        # During training the network outputs logits, and
        # it's up to the user to normalize it before visualising with tensorboard or computing validation metric
...
class UNet3D(Abstract3DUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 num_groups=8, n_levels=4, is_segmentation=True, **kwargs):
        super(UNet3D, self).__init__(in_channels=in_channels, out_channels=out_channels, final_sigmoid=final_sigmoid,
                                     ...
                                     num_groups=num_groups, num_levels=n_levels, is_segmentation=is_segmentation,
                                     **kwargs)
...
class ResidualUNet3D(Abstract3DUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 num_groups=8, n_levels=5, is_segmentation=True, **kwargs):
        super(ResidualUNet3D, self).__init__(in_channels=in_channels, out_channels=out_channels,
                                             ...
                                             num_groups=num_groups, num_levels=n_levels,
...
if __name__ == "__main__":
    ...
    feat_maps = 32
    num_levels = 3
    model = UNet3D(input_channels, output_channels, f_maps=feat_maps, num_levels=num_levels, layer_order='cr')
    print(model)
    ...
    import numpy as np
    import torch
    x_in = np.zeros((1, 1, reso, reso, reso))
    x_in[:, :, int(reso/2-1), int(reso/2-1), int(reso/2-1)] = np.nan
    x_in = torch.FloatTensor(x_in)

    x_out = model(x_in)
    print('%f' % (torch.sum(torch.isnan(x_out)).detach().cpu().numpy()/(reso*reso*reso)))
