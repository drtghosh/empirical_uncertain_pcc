...
class MLPBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.h_nodes = config.mlp_nodes
        self.num_layers = config.mlp_layers
        self.in_dim = config.latent_dim + config.space_dim
        self.out_dim = config.contrast_dim
        self.nonlinear_layer = nn.LeakyReLU()

        # creating the linear (fully connected) layers
        model = [nn.Linear(self.in_dim, self.h_nodes)]
        for i in range(self.num_layers):
            model.append(nn.Linear(self.h_nodes, self.h_nodes))
        model.append(nn.Linear(self.h_nodes, self.out_dim))
...
class ResnetBlockFC(nn.Module):
    ...
    def forward(self, x):
        net = self.fc_0(self.nonlinear_layer(x))
        dx = self.fc_1(self.nonlinear_layer(net))
