...
class TrainerVQVAE(TrainerCommon):
	def __init__(self, config):
		super(TrainerVQVAE, self).__init__(config)
		self.reconstruction_loss = None
		self.quant_loss = None

	def build_model(self, config):
		# customize the build_model function to build the autoencoder
		model = get_model(config, "VQVAE")
	...
	def forward(self, data, train=True):
		...
		self.predicted_pts, loss_quant, _ = self.model(encoded_input_pts)
		if train:
			...
			self.quant_loss = loss_quant
			self.loss = self.reconstruction_loss + self.quant_loss

	def collect_loss(self):
		loss_dict = {
			"comp_recon": self.reconstruction_loss,
			"quantizer": self.quant_loss
			}
		return loss_dict
