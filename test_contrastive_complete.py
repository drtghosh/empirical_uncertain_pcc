...
def test_con_comp():
	...
	save_dir = os.path.join(config.proj_dir,
							"results_comp/ckpt-{}-n{}".format(config.ckpt, saved_test))
	...
	with torch.no_grad():
		for _ in tqdm(range(saved_test)):
			...
			# to use complete data to predict
			...
			grid_data, grid_sizes, corner, spacing = create_grid(trainer.complete_pc.transpose(1, 2), config.grid_size,
												trainer.complete_pc.size(1))
			...
			negative_cloud, negative_label = create_negative_with_label(trainer.complete_pc.transpose(1, 2))
			...
			test_embedding = torch.cat([trainer.complete_embedding, negative_embedding], 1).flatten(0, 1)

			# combine test labels
			test_label = torch.concat((torch.zeros(trainer.complete_pc.size(-1)), negative_label), 0).to(trainer.device)
